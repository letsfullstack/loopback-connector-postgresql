...
function unaccent(sql, queryType) {
  ...
  if (sql.indexOf("~*") !== -1 && sql.indexOf(queryType) !== -1) {
    ...
    var _sqlLeft = [];
    var leftType = "(";
    if (_sql[0].indexOf(leftType) !== -1) _sqlLeft = _sql[0].split(leftType);
    else {
      leftType = "WHERE ";
      _sqlLeft = _sql[0].split(leftType);
    }

    // Split right side from AND or ORDER
    var _sqlRight = [];
    var rightType = " AND \"";
    if (_sql[1].indexOf(rightType) !== -1) _sqlRight = _sql[1].split(rightType);
    else {
      rightType = " ORDER BY ";
      _sqlRight = _sql[1].split(rightType);
    }
    // Merge sql splits
